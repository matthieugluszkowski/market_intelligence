...
Constitution de la liste des ~250 titres : SBF 120 plus les grandes valeurs allemandes, néerlandaises, espagnoles, italiennes et belges. Pour chacun : ISIN, nom, marché, devise, secteur ICB, et **le mapping des symboles vers Stooq et yfinance**.
...
**Acceptation :** 250 instruments en base, chacun avec au moins un symbole vérifié par téléchargement d'une cotation réelle, et zéro doublon d'ISIN.
...
**Acceptation :** ≥ 95% des 250 titres ont ≥ 15 ans d'historique hebdomadaire ; le job relancé deux fois de suite produit un état identique ; l'archive Parquet est relisible.
...
*Note : l'écran matrice qualité × prix arrive avec L6b, pas ici. En attendant, tous les titres s'affichent en `unqualified` - ce qui est leur statut réel et doit se voir.*
...
- **aucun titre ne passe en `solid` sans groupe de pairs contenant un concurrent hors Europe** - c'est le garde-fou contre la limite L1 du doc 08
...
### L7 - Orchestration et rapport hebdomadaire · 2 j
...
|---|---|---|---|
| Mapping de symboles plus long que prévu | **Élevée** | +2 à 3 j sur L1 | Commencer par 50 titres, élargir ensuite |
| Historiques Stooq plus courts qu'annoncé sur certains marchés | Moyenne | Univers réduit | Bascule yfinance au cas par cas |
| Rate limiting yfinance sur le backfill | Élevée | Lenteur | Débit ménagé, cache disque, étalement |
...
**Le risque dominant n'est pas technique, c'est le référentiel.** Constituer et vérifier 250 mappings de symboles est fastidieux, sans intérêt intellectuel, et c'est ce sur quoi le projet peut s'enliser. **Recommandation : faire L1 sur 50 titres, aller jusqu'à L5 avec ces 50, voir le système fonctionner, puis élargir à 250.** Un système qui marche sur 50 titres motive l'élargissement ; un référentiel de 250 lignes à saisir avant de rien voir, non.
